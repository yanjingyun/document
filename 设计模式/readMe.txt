...
com.yjy.strategy2 // 策略模式
	v1版本：
		需求描述：假设鹅厂推出了3种会员，分别为会员，超级会员以及金牌会员，还有就是普通玩家，针对不同类别的玩家，购买《王者农药》皮肤有不同的打折方式，并且一个顾客每消费10000就增加一个级别，那么我们就可以使用策略模式，因为策略模式描述的就是算法的不同，这里我们举例就采用最简单的，以上四种玩家分别采用原价（普通玩家），九折，八折和七价的收钱方式。
	v2版本：
		问题：v1版本的Player类写了会员升级的逻辑，都写在一个方法里面不太优雅，需要进行处理。
		解决：创建一个简单工厂类，用来处理会员升级的逻辑，但这样工厂类的逻辑同样存在ifelse语句。
	v3版本：
		描述：解决v2版本中的工厂类存在ifelse处理
		详情：创建一个价格区间注解类，用于注释每个会员类的价格区间，当前价格在哪个会员类的价格区间则使用哪一个会员类
...
com.yjy.composite // 组合模式（整体-部分模式）
	v1版本，透明组合模式
	v2版本，安全组合模式


com.yjy.broker // 中介者模式
	三种角色：注册中心、抽象服务类、具体服务里
